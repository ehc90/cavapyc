...
export default function BlogPostContent({ post, relatedWine, postId }: { post: BlogPost, relatedWine: WineItem | null, postId: string }) {
    const [likes, setLikes] = useState(post.likes || 0);
    const [hasLiked, setHasLiked] = useState(false);

    useEffect(() => {
        if (localStorage.getItem(`liked_${postId}`)) setHasLiked(true);
    }, [postId]);
