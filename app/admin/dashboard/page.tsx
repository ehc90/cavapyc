...
import { Plus, Edit2, Trash2, Save, X, Sparkles, LogOut, Wine, Upload } from 'lucide-react';
...
export default function AdminDashboard() {
    ...
    const generateBlogAI = async () => {
        if (!blogFormData.image && !blogFormData.title) return alert('Ingresa al menos un título o una imagen.');
        setAiLoading(true);

        const prompt = `Actúa como 'El Príncipe', un viajero sofisticado y sommelier. 
        Escribe una entrada de bitácora (blog) elegante, evocadora y personal.
        Contexto: Título "${blogFormData.title || 'Sin título'}", Etiquetas: "${blogFormData.tags || ''}".
        Analiza la imagen proporcionada (si la hay) y úsala para describir el ambiente, los colores y las sensaciones.
        El tono debe ser culto, apasionado pero sereno. Máximo 300 palabras.`;
        ...
        try {
            const res = await fetch('/api/gemini', {
                ...
                body: JSON.stringify({
                    prompt,
                    image: blogFormData.image // Send image URL if available
                })
            });
            const data = await res.json();
            if (data.text) {
                setBlogFormData((prev: any) => ({ ...prev, content: data.text }));
            }
        } catch (e) {
    ...
    const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            ...
            setBlogFormData((prev: any) => ({ ...prev, image: url }));
        } catch (err) {
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
                    <form onSubmit={handleBlogSubmit} className="bg-slate-900 border border-slate-700 rounded-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto shadow-2xl flex flex-col">
                        ...
                        <div className="p-6 grid grid-cols-1 gap-4">
                            ...
                            <div>
                                <label className="block text-xs uppercase text-slate-500 mb-1">Imagen de Portada (URL o Subir)</label>
                                <div className="flex gap-2">
                                    ...
                                    <label className={`cursor-pointer bg-slate-800 hover:bg-slate-700 border border-slate-700 text-slate-300 px-3 py-2 rounded flex items-center justify-center transition-colors ${uploading ? 'opacity-50 cursor-not-allowed' : ''}`}>
